...
def process_line(input_y, img_array, width, difference_between_lines_for_line_drawing, difference_between_lines, line_height):
                ...
                if white_note:
                    ...
                    if white_note:
                        ...
                        if white_note:
                            ...
                            right = -1
                            if white_note:
                                #going to get a left and right to see where to end the note at
            ...
            if black_note:
                if black_count < difference_between_lines_for_line_drawing * 1.5:
                    ...
                    if black_note:
                        ...
                        up_right = 0
                        while True:
                            temp_pixel_0 = input_y + int(difference_between_lines / 2) - up
                            temp_pixel_1 = x_index - round(black_count / 2) - 1 + up_right
                            temp_pixel = img_array[temp_pixel_0, temp_pixel_1]
                            #for the lines under notes
                            continued = True
                            for new_x_index2 in range (x_index - black_count, x_index):
                                if img_array[temp_pixel_0, new_x_index2] == 255:
                                    continued = False
                                    break
                                if continued:
                                    for new_x_index2 in range (x_index - black_count, x_index):
 ...
 """
                    ...
                    while temp_pixel_above != 255:
                        ...
                        temp_pixel_above = img_array[starting_above_white, x_index - black_count + 1]
                    #going across and finding where it starts and ends for the black_note
                    #the upper start
                    start_up = -1
                    end_up = -1
                    if black_note:
                    ...
                    first = -1
                    middle = -1
                    if end - start <= (difference_between_lines / 2):
                        black_note = False
                    if black_note:
                        for new_x_index in range(start, end):
                            temp_pixel = img_array[input_y, new_x_index]
